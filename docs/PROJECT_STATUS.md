...
| P1 Offline OMS | **PASS** |
| P2 Risk engine | **PASS** |
| P3 Big QMT read-only | **IN PROGRESS — QMT lifecycle/query/callback/file-spool PASS; QMT→spool→Host path observed; Host-only restart replay implemented; real restart/failure calibration and ORDER/DEAL semantics pending** |
...
| Host read model | **IMPLEMENTED — gap/session change remains fail-closed until clean full snapshot resync; Host restart can restore from durable processed snapshot history** |
...
The latest real Host restart exposed an important lifecycle property: a Host process can restart after QMT's initial `bridge_ready` and clean `snapshot` files have already moved from `inbox/` to `processed/`. Because a daily-bar QMT model may not emit another snapshot after market close, consuming only `inbox/` leaves the restarted Host permanently `needs_resync=true`. The Host now replays from the newest clean snapshot in `processed/` before polling `inbox/`. Replay is read-only, does not move/delete source files, preserves account/session/sequence checks, and remains fail-closed if later replayed facts reveal a session change, gap, account mismatch, corruption, or downstream ingestion failure.
