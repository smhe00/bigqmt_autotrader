...
国金模拟 V5 build `p5-simulation-calibration-6` 已解除原先的 BUY-only / 必须 100
...
三种港股路由的证券主数据和精确 tick 证据。build-6 通过大QMT板块接口有界发现
最多 6 个底层港股、20 条诊断 route；发现结果不自动授权交易，生产文件不变。
