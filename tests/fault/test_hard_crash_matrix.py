...
def test_hard_crash_during_submit_never_resubmits(tmp_path, failure_mode, expected_status):
    ...
    row = repo1.get_order_row("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")
    assert row["status"] == OrderStatus.SUBMITTING.value
    assert row["submit_call_started"] == 1
    assert driver.submit_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 1
    ...
    assert repo2.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is expected_status
    assert driver.submit_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 1
    conn2.close()
...
def test_hard_crash_during_cancel_never_recancels(tmp_path, failure_mode, expected_status):
    ...
    with pytest.raises(SimulatedProcessCrash):
        oms1.cancel_order("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")

    row = repo1.get_order_row("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")
    assert row["status"] == OrderStatus.CANCEL_PENDING.value
    assert row["cancel_call_started"] == 1
    assert row["cancel_outcome_resolved"] == 0
    assert driver.cancel_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 1
    ...
    assert repo2.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is expected_status
    assert driver.cancel_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 1
    row = repo2.get_order_row("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")
...
def test_crash_after_submit_reservation_before_driver_call_is_never_reissued(tmp_path):
    ...
    repo1.record_risk_decision("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash", _decision())
    repo1.prepare_submit("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")
    assert repo1.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is OrderStatus.SUBMITTING
    assert driver.submit_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 0

    conn1.close()
    conn2, repo2, _ = _restart_after_crash(path, driver, clock)

    assert repo2.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is OrderStatus.MANUAL_REVIEW
    assert driver.submit_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 0
    conn2.close()
...
def test_crash_after_cancel_reservation_before_driver_call_is_never_recancelled(tmp_path):
    ...
    repo1.prepare_cancel("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")
    assert repo1.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is OrderStatus.CANCEL_PENDING
    assert driver.cancel_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 0
    ...
    assert repo2.get_status("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") is OrderStatus.ACKNOWLEDGED
    assert driver.cancel_call_count("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash") == 0
    assert repo2.get_order_row("sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "cid-hard-crash")["cancel_outcome_resolved"] == 1
    conn2.close()
