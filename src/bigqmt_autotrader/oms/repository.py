...
class OmsRepository:
    ...
    def register_qmt_durable_submit(
        ...
        *,
        ...
        The imported order starts in RECONCILING: the command file proves only
        that a side effect may have crossed the bridge boundary.  It is never
        broker ACK evidence.
        """
        identity = (
        ...
        with transaction(self.conn):
            ...
            if row is not None:
                ...
                if (
                    ...
                    self.conn.execute(
                        """
                        UPDATE qmt_durable_command_identities SET command_state='processed'
                        WHERE command_id=?
                        """,
                        (command_id,),
                    )
                ...
                if stored != identity:
                    raise QmtDurableIdentityConflict(
                        "conflicting durable QMT command identity"
                    )
                return False
